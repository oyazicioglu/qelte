<script lang="ts">
	...
	export let menuIcon: string = undefined;
	...
	const useIconButton = menuIcon && !text;
	const id = createUId();
...
<div
	...
	style={styles}>
	{#if useIconButton}
		<IconButton circle={circledIcon} on:click={toggleMenu} {type} {color} {size} iconName={menuIcon} {disabled} />
	{/if}

	{#if !useIconButton}
		<Button on:click={toggleMenu} {type} {color} {size} {disabled} {rounded} gap="2">
			{#if menuIcon}
				<Icon iconName={menuIcon} {type} color={menuIcon && !text ? color : 'inherit'} {size} {disabled} />
			{/if}
